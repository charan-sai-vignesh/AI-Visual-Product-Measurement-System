...
```
Excel Dataset → Data Loader → Image URLs
                                   ↓
Image URLs → Image Processor → Downloaded Images
                                   ↓
Images → Vision Analyzer → Text Descriptions
                                   ↓
Descriptions → Measurement Extractor → Structured Measurements
                                   ↓
Measurements → API → JSON Response / Frontend Display
```
...
3. **Optional: Configure Hugging Face API Key** (for better performance)
   ...
   **Note**: The system works without an API key using local models, but it's slower and requires more setup.

4. **Ensure the dataset file is in the root directory**:
...
├── README.md                        # This file
├── EXPERIENCE.md                    # Development experience
├── A1.0_data_product_images.xlsx   # Dataset
├── src/
│   ├── __init__.py
...
│   │   ├── vision_analyzer.py       # Vision AI integration
│   │   ├── measurement_extractor.py # Measurement extraction
│   │   └── product_analyzer.py      # Main analyzer
│   └── api/
